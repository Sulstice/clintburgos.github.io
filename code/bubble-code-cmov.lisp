...
loop2
	...
	(cmovl		%esi		%eax)	;;t = *(Fptr + 1)
	(cmovl		%edi		%esi)	;;*(Fptr + 1) = *Fptr
	(cmovl		%eax		%edi)	;;*Fptr = *(Fptr + 1)
	
	(rmmovl		%edi	0(%edx))	
	(rmmovl		%esi	4(%edx))	;;put into memory
...
(! init-mem
    (hons-shrink-alist
     (y86-asm (@ bubble-code) (@ location) (@ symbol-table) 'sum)
     'shrunk-sum))
